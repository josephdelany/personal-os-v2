...
| ADR-0016 | Analytical store — Postgres authoritative; Parquet-on-R2 the analytical mirror + scratch, DuckDB in the Actions runner (hosted Supabase cannot run pg_duckdb/pg_ducklake, confirmed on the live instance); results written back with provenance (INV-1/RULE-12 stay inside one enforceable store). Must reference OQ-20 (500 MB fill). *Reserved 2026-08-23, Phase-2 planning; see `docs/PHASE2_MIGRATION_PLAN.md`.* |
| ADR-0017 | Non-response as a row — a `prompt_dispatch` table (one row per prompt issued; `response_state ∈ {answered, seen_declined, delivered_unseen, partial, expired}`); `never_scheduled` is the absence of a row. Distinct axis from RULE-07 presence. *Reserved 2026-08-23.* **Citation note:** JMIR e65350 is **snippet-confirmed, primary unreachable** (403) — the decision stands on its own reasoning, not the citation; label accordingly in the ADR. |
| ADR-0018 | Metric-registry scale/rounding metadata — `response_scale`, `n_scale_points`, `rounding_step`, `self_report` per variable, so self-reports are stored as coarsened interval data. *Reserved 2026-08-23.* |
| ADR-0019 | Temporal amendment to ADR-0002 — instant + `time_precision` for point events, `valid_interval tstzrange` for durational atoms, `expired_at` for transaction time; `subject_day` moves from a *generated* column to an application-computed *stored* column with `rule_version` (04:00 by start instant, sleep by wake day). Amends RULE-03. *Reserved 2026-08-23; resolves OQ-06.* |
| ADR-0020 | Trust/egress — `trust_level ∈ {trusted, untrusted}` on `atoms`/`raw_captures` set at ingest; a session that reads personal rows must not also hold an egress capability (and vice-versa); no generic `execute_sql` reaches a model. RULE-29 clarification, no new numbered rule (30-cap holds). *Reserved 2026-08-23.* **Citation note:** NSA CSI U/OO/6030316-26 is **snippet-confirmed, primary unreachable** (403); the decision stands on the lethal-trifecta reasoning (Willison, verified) + OWASP LLM01/LLM06, not the NSA doc; label accordingly in the ADR. |
| ADR-0021 | Distributional forecasts — continuous predictions store a `forecast_distribution` (quantiles/samples) + `issued_at`/`horizon`/`resolves_at` and `crps`/`log_score_cont`; binary path (`p_forecast`/`brier`/`log_score`) kept. Extends RULE-20. *Reserved 2026-08-23; split from ADR-0016 to fix a double-booked number.* |

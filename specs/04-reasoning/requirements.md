...
**REQ-INF-412 (Event-driven)**
WHEN an on-demand exploratory run is requested — by Joe or by the orchestration layer — the reasoning layer SHALL execute the requested run at that time, independent of the weekly and monthly scheduled cadence (RULE-19 SCOPE shell: exploration is continuous and may run at any time). An on-demand run is a hypothesis-generating pass — a generator method (REQ-INF-400) or a probabilistic-inference pass (REQ-INF-520) invoked as a hypothesis generator — and SHALL NOT be the confirmation job or a regime-model fit (REQ-INF-540). The reasoning layer SHALL derive the run's specification entirely from `metric_registry` and `v_metric_tree` (REQ-INF-500; RULE-13 — the specification is fixed data, never chosen by the model at query time), SHALL apply the RULE-17 binding-sequencing gate (REQ-TIER-051 — an on-demand run is exploration and does not run before the `EXPLORATORY` surface is built and proven), and SHALL apply every precondition and floor of the method it invokes exactly as the scheduled run of that method would — for a generator run: block size (REQ-INF-404), the coverage floors (REQ-INF-405, REQ-INF-406), detrend/deseasonalize (REQ-INF-409), and circular-shift null calibration (REQ-INF-410). (Missing-G: this is the on-demand guarantee RULE-19's SCOPE shell asserts and no prior REQ-INF provided — every generator run was weekly/monthly batch. It adds the *trigger*, not a new method, a new tier, or a cadence number; how often the orchestration layer fires an on-demand run is an operational parameter, not fixed here.)

**REQ-INF-413 (Ubiquitous)**
The reasoning layer SHALL write an on-demand run's output into `hypothesis_register` with status `CANDIDATE`, exactly as a scheduled generator run does (REQ-INF-401) and regardless of whether a generator method or a probabilistic-inference pass produced it; SHALL make that output displayable only on the built-and-proven `EXPLORATORY` surface, which sources `CANDIDATE` rows and renders no row of any other status (REQ-TIER-053, gated by the REQ-TIER-051 binding sequencing); and SHALL NOT create a `findings` row from it, advance any hypothesis to `PROMOTED` or above on its evidence, or trigger the confirmation job. The on-demand capability is exploration only: the confirmation job continues to read solely observations whose `subject_day` and `ingested_at` are at or after `confirmation_data_from` (REQ-INF-104), which is set to `now()` at registration (REQ-INF-101), so an on-demand run over data that pre-existed a hypothesis's registration cannot confirm it whatever fires the confirmation job — RULE-19's waiting clock is untouched by the on-demand capability.

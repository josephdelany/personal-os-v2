...
### §0 requirements
```
**REQ-FIN-001** (Ubiquitous) The finance subsystem SHALL persist every transaction as an `atom` of kind 'transaction' in the shared atoms table, carrying — where it stores an amount — that value's lane as `estimate_method` and `state_class` (RULE-05, the `atoms_value_has_lane` constraint), plus `confidence` and `provenance`, so that spend is queryable on the same `subject_day` axis as every other lens. (Column names reconciled to the built atoms table 2026-08-24, requirements-audit C-8 / OQ-26: `lane`→`estimate_method`+`state_class`, `atoms.local_date`→`subject_day`; `source` dropped — every atom already traces to its capture via the `NOT NULL raw_capture_id` (INV-1), so a separate `source` column was never real. Migration 0005.)
...
**REQ-FIN-026** (Optional feature) WHERE the Cloudflare Workers AI fallback is enabled, the ingest layer SHALL invoke it only for emails that failed template parsing, and SHALL mark any resulting `raw_transactions` row `provenance = 'inferred'` with `confidence` set to the model's reported value. (C-8: `lane`→`provenance` aligns the vocabulary to the spine; `raw_transactions` is an unbuilt Phase-3 table, so this is a *forward* commitment to the spine's `provenance` vocabulary, not a reconciliation to an existing column.)
...
**REQ-FIN-114** (Ubiquitous) Every usage status SHALL carry `provenance = 'inferred'` and a `confidence` value unless it was set directly by Joe, in which case it SHALL be a first-class human row (RULE-10) carrying `confidence = 1.0` and superseding any inferred status for that subject. (C-8: the spine has no `lane` column and no `'hard'` value; the inferred case maps cleanly to `provenance = 'inferred'`, migration 0005. **The exact provenance representation of a *direct human override* is deliberately NOT decided here — it is OQ-32.** The old `lane='hard'` carried the authoritative-override semantic in a dedicated field; the spine's `provenance` enum {extracted, inferred, defaulted} has no distinct "human override" value, so whether the override is `extracted`+confidence+supersedes-lineage or needs its own marker is a modeling decision for Joe, not a spelling reconciliation.)
...
**REQ-FIN-198** (Optional feature) WHERE a bar tab amount is used as a prior on drink count, the finance subsystem SHALL store it with `provenance='inferred'` and `confidence` no greater than 0.30, SHALL present it with that provenance and confidence disclosed and SHALL NOT present it as an established drink count, and SHALL discard it the moment an actual `consume` atom exists for that episode (RULE-25; C-8: `lane`→`provenance`).
...
**REQ-FIN-214** (Ubiquitous) The finance subsystem MAY define, store and display a budget or target for a category, and SHALL render every budget surface as retrospective or range-based and never as a live running counter, remaining bound by REQ-FIN-210 (no running spent/remaining count), REQ-FIN-211 (no figure updating more than once per 24 hours) and REQ-FIN-212 (forward amounts as ranges). (Reworded 2026-08-24, requirements-audit C-3 / RULED-2 / ADR-0031: the former per-category budget ban is reversed — a full finance system includes budgets — but the surviving constraint is that a budget is never a live number ticking down; the $32–40 over-spend evidence is about precise always-on feedback, which REQ-FIN-210/211 forbid. The budget-setting and budget-vs-actual machinery is authored under Missing-C, and MUST NOT become a scolding tally or a compliance score, RULE-23/24.)
...
**REQ-FIN-218** (Ubiquitous) The finance subsystem SHALL NEVER emit any of the following words in reference to Joe's spending: 'excessive', 'wasteful', 'necessary', 'unnecessary', 'too much', 'splurge', 'guilty', 'overspent', 'bad', 'should have'. (C-10, 2026-08-24: added 'necessary' to match RULE-23's necessary/unnecessary pair and to keep this finance banlist consistent with the reasoning-layer REQ-NAR-023 wordlist — the finance copy path had the same gap.)
...
- **Shorter budget windows and roll-over reminders.** These are listed in the research among interventions that *did* help. Budgets are now in scope (REQ-FIN-214, reversed by RULED-2/ADR-0031; machinery owed under Missing-C), so the former "no budget exists in this design" rationale no longer holds; roll-over *reminders* specifically are still deferred because a per-period reminder is a recurring nag that fits poorly with the low-frequency-review architecture (REQ-FIN-211, REQ-FIN-213). Recorded as a genuine open option if Joe later wants one.
